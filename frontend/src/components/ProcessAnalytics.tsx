...
export const ProcessAnalytics = () => {
    ...
    const [analytics, setAnalytics] = useState({
        modelAccuracy: 98.29,
        recommendationsToday: 14,
        successfulAdditions: 13,
        costSavings: 845.50,
        avgResponseTime: 2.1,
        systemUptime: 99.95
    });
    ...
    const loadAnalytics = () => {
        fetch('/api/charts/process-analytics/')
            ...
            .then(data => {
                setAnalytics(prev => ({
                    ...prev,
                    recommendationsToday: data.recommendations_today,
                    successfulAdditions: data.successful_additions,
                    costSavings: data.cost_savings,
                    avgResponseTime: data.avg_response_time,
                    systemUptime: data.system_uptime
                }));
            })
            .catch(err => console.error("Error loading process analytics:", err));
    };
        ...
        const loadAccuracy = async () => {
            try {
                ...
                if (data) {
                    setAnalytics(prev => ({
                        ...prev,
                        modelAccuracy: data.averageAccuracy
                    }));
                }
            } catch (error) {
