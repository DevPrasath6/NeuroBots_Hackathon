...
exports.createReading = factory.createOne(SpectrometerReading, ['reading_id']);
exports.updateReading = factory.updateOne(SpectrometerReading, ['reading_id']);
...
exports.requestOPCReading = catchAsync(async (req, res, next) => {
  ...
  try {
    // Validate or resolve metal grade flexible match
    const MetalGradeSpec = require('../models/metalGradeModel');
    const escapeRegex = require('../utils/escapeRegex');
    const safeGrade = escapeRegex(metalGrade);
    let gradeExists = await MetalGradeSpec.findOne({
      metal_grade: { $regex: new RegExp(`^${safeGrade}$`, 'i') },
    });

    if (!gradeExists) {
      gradeExists =
        (await MetalGradeSpec.findOne({
          metal_grade: { $regex: new RegExp(safeGrade, 'i') },
        })) || (await MetalGradeSpec.findOne({}));
    }

    const targetGrade = gradeExists ? gradeExists.metal_grade : metalGrade;
