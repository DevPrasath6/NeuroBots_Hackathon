...
const ALLOWED_ROLES = ['user', 'admin', 'operator', 'supervisor'];
...
exports.getCurrentUser = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    console.warn('Firebase Admin getUser warning:', error.message);
    res.status(200).json({
      status: 'success',
      data: {
        user: {
...
exports.sendPasswordResetEmail = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // TODO: wire up an email provider to actually send `link` to the user's inbox.
    // The link must never be returned in the API response - anyone who knows a user's
    // email would otherwise be able to fetch a working account-takeover link for them.
...
// Set custom claims (for role-based access) - admin only, enforced by restrictTo('admin') on the route
exports.setUserRole = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // Keep MongoDB metadata (the role source of truth used by the auth middleware) in sync
