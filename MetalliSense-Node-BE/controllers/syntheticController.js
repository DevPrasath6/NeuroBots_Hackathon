...
exports.generateSyntheticReading = catchAsync(async (req, res, next) => {
  ...
  // Get a sample from training data for this grade or fallback to gradeSpec ranges
  const escapeRegex = require('../utils/escapeRegex');
  let baseSample = await TrainingData.findOne({
    grade: { $regex: new RegExp(`^${escapeRegex(metalGrade)}$`, 'i') },
  });

  if (!baseSample) {
    const ranges = gradeSpec.composition_ranges || {};
    const getMid = (r) => (Array.isArray(r) && r.length === 2 ? (r[0] + r[1]) / 2 : 1.0);
    baseSample = {
      Fe: getMid(ranges.Fe || [82.0, 90.0]),
      C: getMid(ranges.C || [3.0, 4.0]),
      Si: getMid(ranges.Si || [1.8, 2.8]),
      Mn: getMid(ranges.Mn || [0.3, 1.0]),
      P: getMid(ranges.P || [0.01, 0.08]),
      S: getMid(ranges.S || [0.01, 0.03]),
    };
  }
