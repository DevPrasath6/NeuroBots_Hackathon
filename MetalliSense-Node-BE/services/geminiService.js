...
try {
  ...
  console.warn('⚠️ ElevenLabs SDK load warning:', err.message);
}

class GeminiExplanationService {
  constructor() {
    this.apiKey = process.env.GROQ_API_KEY || process.env.GOOGLE_GEMINI_API_KEY;
    this.elevenlabsApiKey = process.env.ELEVENLABS_API_KEY;
    
    if (!this.apiKey) {
      console.error('GROQ_API_KEY / GOOGLE_GEMINI_API_KEY not found in environment variables');
      this.groq = null;
    } else {
    ...
    if (!this.elevenlabsApiKey || !ElevenLabsClient) {
      console.warn('ELEVENLABS_API_KEY not found or SDK unavailable - TTS will be disabled');
      ...
      try {
        ...
        console.warn('ElevenLabs client initialization failed:', err.message);
        this.elevenlabs = null;
      }
    }
