...
# API settings
# Defaults to localhost-only; set API_HOST=0.0.0.0 explicitly (e.g. in a container) to expose on the network
...
# Comma-separated list of allowed origins for CORS; "*" is rejected when credentials are allowed
_default_origins = "http://localhost:5173,http://localhost:3000,http://localhost:3000"
ALLOWED_ORIGINS = [
    origin.strip()
